...
namespace lae\Client\Interceptor;
...
use \lae\Client\Interceptor\MyLogger;
use \lae\Client\Model\Errors;
use \lae\Client\Model\Error;

class MiddlewareEvents
{
    
    function __construct(\lae\Client\Interceptor\KeyHandler $signer)
    {
        $this->signer = $signer;
...
function build_error($code, $message){
    $error = new \lae\Client\Model\Error([
        "code" => $code,
        "message" => $message
    ]);
    $errors = new \lae\Client\Model\Errores(["errors" => [$error]]);
